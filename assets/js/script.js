...
window.onload = () => {
	...
	buildGui = (object, opts) => {
		gui.innerHTML = `
			<div class="item content" style="margin-top: 0px;"><p class="ttle">Message content</p></div>
			<div class="edit">
			...
			<!-- kerenn -->
			<div class="item title inlineField">
				<input id="getbottoken" class="editBotToken" style="width: 90%;" type="text" placeholder="BOT-TOKEN" autocomplete="off" maxlength="256">
			</div>
			<div class="item title inlineField" style="color: #44c383;">
				<input id="applycontinue" class="editBotToken" type="button" onclick="apply_continue()" value="Apply & start bot on 10 sec" autocomplete="off" maxlength="256">
			</div>`;
...
function apply_continue() {
	document.querySelector('#applycontinue').value = 'vm bot started'
	setTimeout(() => {
	document.querySelector('#applycontinue').value = 'Apply & start bot on 10 sec';
	}, 5000);

	token = document.querySelector('#getbottoken').value;
	vm_bot_start(token)
}